...
class DataAnalyzer(Resource):
    def post(self):
        ...
        if args['filePath']:
            ...
            tools.InsertingDataFrame(csvdata)
            # print(csvdata.info())
            response_data = DataTypeAnalyzer.column_stats(csvdata) # call external function
...
class GetMinMax(Resource):
    def post(self):
        ...
        args = parser.parse_args()  # parse arguments to dictionary
        tools = InsightGeneration.InsightGeneration()
        data = tools.minmaxfinder(args['colName'])
        print(args['colName'])
        print(data)
...
class CreateGroup(Resource):
    def post(self):
        ...
        global projectDF # This global need to remove, should use utils to save DF
        tools = InsightGeneration.InsightGeneration()
        projectDF = tools.CreateGroup(args['colName'],thList,grpList)
...
class CategoricalImpute(Resource):
    def post(self):
        ...
        projectDF = ImputeCategorical.ImputeCategorical(projectDF,target,colNames)
        print("output: OK")
        return {'data': "OK"}, 200  # return data and 200 OK
...
class GeneratorRank(Resource):
    def post(self):
        ...
        rank = FeatureRank.FeatureRank(projectDF,target,colNames)
